...
ggplot(dat_top, aes(year, total, colour=chapter, group=interaction(chapter, location))) +
  ...
  labs(title = "Number of R-Ladies Events for Chapters with the 10 highest number of Events",
       subtitle = "Solid lines are in person events, while dashed lines are online events",
       caption = "Data from https://github.com/rfordatascience/tidytuesday") +
  labs(tag="stingh@mstdn.party") +
